...
import { getEnvVariablesDon, getRandomCity, RequestData, BatchResult, avg } from "../utils/helper";
...
async function runBatch(
	...
	for (let i = 0; i < batchSize; i++) {
		...
		const txPromise = consumer
			.requestTemperature2(getRandomCity(), JOB_IDS, {
	...
	const createdFilter = consumer.filters.RequestCreated2();
	const fulfilledFilter = consumer.filters.RequestFulfilled2();
